...
- **재생/정지 토글**: 선택한 리스트가 재생(일시정지 포함) 중이면 재생을 시작한 듣기 버튼(셔플듣기/전체듣기)에 문구 뒤 정지 아이콘이 표시되어 어느 모드로 재생 중인지 구분되고, 그 버튼을 클릭하면 바로 정지. 재생 중인 곡의 행 버튼도 정지 버튼으로 바뀜(정지·곡 전환 시 원복) (FR-18)
