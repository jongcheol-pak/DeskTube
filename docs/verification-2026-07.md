...
**시나리오 A — 창 숨김 경로** (유휴 상태에서 X 닫기 → 트레이 숨김):
...
- 앱 내부 로그: `워킹셋 트림 완료: 196MB → 2MB` (직후 재적재분 포함 외부 측정 약 26MB에 수렴)

**시나리오 B — 재생 정지 경로** (UIA로 URL 재생 → 19초 영상 종료 → 순차 모드 자동 정지 = StopAsync):

| 시점 | 워킹셋 (외부 Get-Process 측정) |
|---|---|
| 재생 중 (배경창 부착 후) | 211.2 MB |
| 정지(영상 종료) 후 | 27.3 MB |

- 앱 내부 로그: `워킹셋 트림 완료: 210MB → 12MB`. 재생 구간(부착~종료 32초)에는 트림 로그 없음
  — **재생 중 트림 미호출(acceptance ③)도 로그로 검증됨**
- **NFR-2 목표(정지 대기 ≤150MB) 대비: 충족** — 두 경로 모두 트림 후 약 27MB (목표의 1/5 수준)
...
- [ ] ⏳ HUMAN-VERIFY: 트림 직후 재생 재시작 시 체감 지연 없음 (문제 시 숨김 시점 트림만 유지 — plan D6 Edge)
