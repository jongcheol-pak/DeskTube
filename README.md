...
- **부팅 자동 시작**: Windows 로그인 시 창 없이 트레이로 시작해 마지막 재생 설정으로 자동 재생 — 마지막으로 재생하던 항목부터 이어서, 신규 설치는 기본 켬 (FR-8)
...
- **앱 시작 후 자동 재생**: 설정 토글(기본 켜짐) — 켜면 앱을 열 때 마지막으로 재생하던 항목부터 배경 재생을 자동 시작. 마지막 재생이 홈 즉시 재생("빠른 재생")이면 자동 재생하지 않음(부팅 자동 시작에도 동일 적용) (FR-19)
