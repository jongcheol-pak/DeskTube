...
- **배경 재생**: 유튜브 URL(watch / youtu.be / shorts / embed / live)을 입력하면 바탕화면 배경에서 재생 — 홈의 마지막 입력 URL은 저장되어 재실행 시 입력란에 그대로 표시, 재생 불가 영상(삭제·비공개·임베드 금지)은 자동으로 다음 항목으로 건너뛰고 전 항목이 재생 불가면 정지 후 안내 (FR-1)
